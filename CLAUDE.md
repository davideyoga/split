...
- Sections so far: `add-expense.*`, `select-participant.*`, `login.*`, `home.*`, `groups.*`, `categories.*` (the latter holds both UI labels and one key per preset category slug). Every component is fully translated (the `select-participant` modal's old hardcoded Italian was converted to `select-participant.*` when the person/group toggle landed).
