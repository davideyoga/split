...
**Frontend (`splitFront`):**
- Standalone Angular components/pages under `src/app/pages/` (`home`, `add-expense`, `groups` — list + inline create-group form, `groups/group-detail` — rename + add/remove members) and `src/app/components/` (`select-participant`). Routes `groups` and `groups/:publicId` are lazy `loadComponent` + `authGuard`, like the rest.
...
- Cross-cutting state is handled via services in `src/app/services/`: `participant-selection-service.ts` passes the picked `User` from the `select-participant` modal to whoever opened it (`add-expense`, and the `groups` pages for member selection) via a plain RxJS `Subject`; `user.service.ts` / `group.service.ts` / `expense.service.ts` talk to the backend. (`data-sharing.ts` was dead legacy code — deleted 2026-08-30.)
...
- Sections so far: `add-expense.*`, `login.*`, `home.*`, `groups.*`. (The `select-participant` modal still has hardcoded Italian strings — to be converted to a `select-participant.*` section in Phase 2 when the person/group toggle is added.)
